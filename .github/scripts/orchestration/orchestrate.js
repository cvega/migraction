    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        if (!batchCompleted && attempts % 20 === 0) { // Every 10 minutes
            ...
            await github.rest.issues.createComment({
                ...
                owner: context.repo.owner,
                repo: context.repo.repo,
                body: `:clock3: Batch ${batchNumber} still in progress... (${minutes} minutes elapsed)\n\n:stop_sign: To cancel, add a comment with \`/cancel-migration\``
            });
            }
        }
        }
        ...
        if (completedRun) {
            ...
            await github.rest.issues.createComment({
                ...
                owner: context.repo.owner,
                repo: context.repo.repo,
                body: `:warning: Batch ${batchNumber} of ${batches.length} - workflow not found\n\nBatch ID: ${batchId}\n\n_Moving to next batch after timeout._`
            });
        } else {
            // Timed out
            console.log(`WARNING: Batch ${batchNumber} timed out after 12 hours`);
